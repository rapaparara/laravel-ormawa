                    ...
                    <tbody>
                        @foreach ($dataKepengurusan as $key => $value)
                            <tr class="bg-white border-b ">
                                ...
                                @php
                                    $editAction = session('user_role') == 'mahasiswa' ? 'edit' : 'ganti';
                                    $editLabel = session('user_role') == 'mahasiswa' ? 'Edit' : 'Ubah Status';
                                @endphp
                                <td class="px-3 py-2">
                                    <div class="flex flex-wrap gap-2">
                                        @if ($value->status == 'belum')
                                            <a wire:click.prevent="{{ $editAction }}('{{ $value->id }}')"
                                                class="action-button bg-yellow-300 text-white font-bold py-2 px-3 rounded-lg hover:bg-yellow-400 focus:ring-4 focus:outline-none focus:ring-yellow-200 transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-sm"
                                                href="#"
                                                data-modal-toggle="{{ $editAction == 'edit' ? 'tambah-modal' : 'ganti-modal' }}">
                                                <i class="me-2 fa-solid fa-pencil"></i>{{ $editLabel }}
                                            </a>
                                        @else
                                            <span
                                        ...
                                        @endif
                                        @if (session('user_role') == 'mahasiswa' && $value->status == 'belum')
                                            <a wire:click.prevent="delete('{{ $value->id }}')"
                                                class="action-button bg-red-600 text-white font-bold py-2 px-3 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 transition duration-300 ease-in-out transform hover:scale-105 hover:shadow-sm"
                                                href="#" data-modal-target="hapus-modal"
                                                data-modal-toggle="hapus-modal">
                                                <i class="me-2 fa-solid fa-trash"></i>Hapus
                                            </a>
                                        @endif
                                    </div>
                                </td>
                ...
                <div wire:ignore.self class="p-4 md:p-5">
                    ...
                    <div>
                        <label for="file_sk" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">File
                            SK</label>
                        @if ($editModal == true)
                            <input type="file" wire:model="temp_file_sk" accept=".pdf"
                        ...
                        <div wire:loading wire:target="temp_file_sk"
                            class="px-3 py-1 text-sm font-medium leading-none text-center text-green-800 bg-green-200 rounded-lg animate-pulse">
                            Sedang mengupload file...</div>
                    </div>
                    <button type="submit"
